...
const IndividualTaskOperations = ({ element }) => {
    const {projects,selectedProject,tasks,setTasks} = useContext(StateContext)
    ...
    <>
      {!editable && (
        <div className="flex justify-between group ">
          <Checkbox key={element.id} value={element.id} className="rounded-full" onChange={handleChecked}>
      ...
      {
        editable && (
            <AddIndividualTask projects={projects} onCancel={handleCancel}  selectedProject={selectedProject} initial={element}/>
        )
      }
    </>
  );
